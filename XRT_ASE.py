...
if os.system('which xselect') != 0:
    print 'xselect not found. Is HEASoft initiated?'
    sys.exit(1)
...
for i in range(len(file_names)):
    ...
    if extr_flag is True:
        if splt_flag is False:
            ...
            print 'Looking for region files'
            while (os.path.isfile(file_names[i].replace('cl.evt', 'src.reg')) is False) or (os.path.isfile(file_names[i].replace('cl.evt', 'bkg.reg')) is False):
                print 'Region file(s) not found.'
...
######
        if splt_flag is True:
            ...
            for j in range(len(evt_fits['GTI'].data)):
                ...
                print 'Looking for region files'
                while (os.path.isfile(file_names[i].replace('cl.evt', 'seg'+str(j+1)+'_src.reg')) is False) or (os.path.isfile(file_names[i].replace('cl.evt', 'seg'+str(j+1)+'_bkg.reg')) is False):
                    print 'Region file(s) not found.'
                    print 'Create and save source and backgroud regions'
